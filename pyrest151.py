#
# Programa desenvolvido para o atendimento de clientes e outros serviços em restaurantes.
# 
#Desenvolvido por: Danrley Daniel
# Instagram: @danrleydaniel
# Email: danrleydaniel21@gmail...
#
#VERSÃO INACABADA.
#
...
##### FUNÇÕES DE VALIDAÇÃO, GERAÇÃO DE ID E COISAS SEMELHANTES #####
	
def validaExpressao(x):
	aux = x.split(" ")
	if aux[0].isnumeric() == False:
		return False
	elif aux[2].isnumeric() == False:
		return False
	else:
		return True
...
def pedido():
	...
	arqNt = open("ntsFiscais.dat","wb")
	pickle.dump(notas, arqNt)
	arqNt.close()
...
def calculadora():
	...
	while (validaExpressao(expr) == False):
		expr = input("Expressão inválida: ")
	lis = expr.split(" ")
	if (lis[1] == "+"):
		num1 = float(lis[0])
		num2 = float(lis[2])
		...
		print("Alguma coisa deu errada. Verifique novamente a expressão digitada.")
...
try:
	arqNt = open("ntsFiscais.dat","rb")
	notas = pickle.load(arqNt)
	arqNt.close()
except IOError:
	print()
